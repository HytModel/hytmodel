...
class SellerService {
    ...
    calculateSellerAmounts(priceCents, isAffiliated) {
        const commissionRate = isAffiliated ? 0.10 : 0.15;
    ...
    async payoutToSeller(seller, sellerAmount, paymentIntent) {
    ...
    async processSeller(item, paymentId, paymentIntent) {
        ...
        const { commission, sellerAmount } = this.calculateSellerAmounts(priceCents, seller.is_affiliated);

        // Facture vendeur
        await this.createSellerInvoice(seller, priceCents, commission, sellerAmount);

        // Paiement Stripe Connect
        const transfer = await this.payoutToSeller(seller, sellerAmount, paymentIntent);
